...
riscv64_back_trace_cmd(struct bt_info *bt)
{
	...
	for (;;) {
		...
		/* When backtracing to do_irq(), restore FP/SP and switch to process stack */
		...
		if ((bt->flags & BT_IRQSTACK) &&
			...
			if (riscv64_on_process_stack(bt, current.fp)){
				...
				if (!readmem((ulong)frameptr, KVADDR, &curr_frame,
				      sizeof(curr_frame), "get irq stack frame", RETURN_ON_ERROR))
					return;

				current.fp = curr_frame.fp;
				current.sp = current.fp - 16;
...
static void
riscv64_print_exception_frame(struct bt_info *bt, struct riscv64_register *regs, int mode)
{
	...
	for (ptr = bt->stackbase; ptr < bt->stacktop - SIZE(pt_regs); ptr++) {
		...
		if (riscv64_is_kernel_exception_frame(bt, ptr)){
			if (!old_regs || (old_regs &&
				...
				riscv64_print_exception_frame(bt, regs, KERNEL_MODE);
				count++;
			}
		}
	}
...
riscv64_eframe_search(struct bt_info *bt)
{
	...
	riscv64_print_exception_frame(bt, regs, USER_MODE);
